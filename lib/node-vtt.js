  ...
  Phantom.create(function(error, ph) {
    ...
    ph.createPage(function(error, page) {
      ...
      page.open(options.uri, function(error, status) {
        ...
        self.phantom = ph;
        // Call _setupParser which will make sure our state is clean and that an
        // instance of WebVTT.Parser is attached to the page's window.
        _setupParser(self.page, self.options.encoding, onInit);
      });
    });
  });
};
...
NodeVTT.prototype.clear = function(onClear) {
  if(!_checkReady(this.ready, onClear)) {
    return;
  }
  this.cues = [];
  this.regions = [];
  this.errors = [];
  _setupParser(this.page, this.options.encoding, onClear);
};
...
NodeVTT.prototype.setupParser = function(type, onSetup) {
  if (!_checkReady(this.ready, onSetup)) {
    return;
  }
...
NodeVTT.prototype.parse = function(data, onParsed) {
  if (!_checkReady(this.ready, onParsed)) {
    return;
  }
...
NodeVTT.prototype.flush = function(onFlush) {
  if (!_checkReady(this.ready, onFlush)) {
    return;
  }
...
NodeVTT.prototype.processParsedData = function(data, onProcessed) {
  if (!_checkReady(this.ready, onProcessed)) {
    return;
  }
